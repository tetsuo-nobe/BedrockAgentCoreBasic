...
* 下記のファイルを開き、すべての内容をメモしておきます。
  - `BedrockAgentCoreBasic/gateway/.env`
...
1. **検出 URL** に .env の DISCOVERY_URLの値を入力します。
1. **クライアント ID** .env の CLIENT_ID の値を入力します。
1. **クライアントシークレット** .env の CLIENT_SECRERT の値を入力します。
